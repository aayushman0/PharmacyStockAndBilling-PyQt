...
class ServiceBillWindow(QWidget):
    def __init__(self, service_bill_id: int):
        super(ServiceBillWindow, self).__init__()
        self.service_bill_id = service_bill_id
        loadUi("UI/service_bill.ui", self)
        self.load_pharma_info()
        self.load_service_bill()
    ...
    def load_pharma_info(self):
        env = dotenv_values(".env")
        self.pharmacy_name.setText(env.get("CLINIC_NAME"))
        self.pharmacy_address.setText(env.get("PHARMACY_ADDRESS"))
        self.pan_no.setText(f'PAN No.: {env.get("PAN_NO")}')
        self.dda_no.setText(f'DDA No.: {env.get("DDA_NO")}')
    ...
    def load_service_bill(self):
        ...
        self.bill_no.setText(f"Bill no.: {bill.id}")
        self.date.setText(f"Date: {bill.bill_date.strftime('%Y/%m/%d %H:%M')}")
        self.customer_name.setText(f"Name: {bill.patient_name}")

        self.table_bill_print.setRowCount(0)
        for i, row in enumerate(json.loads(bill.bill_json)):
            self.table_bill_print.insertRow(i)
            self.table_bill_print.setItem(i, 0, QTableWidgetItem(row.get("particular")))
            self.table_bill_print.setItem(i, 1, QTableWidgetItem(str(row.get("quantity"))))
            self.table_bill_print.setItem(i, 2, QTableWidgetItem(str(row.get("price"))))
            self.table_bill_print.setItem(i, 3, QTableWidgetItem(str(row.get("total"))))
        self.table_bill_print.resizeColumnsToContents()
